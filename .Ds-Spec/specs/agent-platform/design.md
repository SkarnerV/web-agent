...
5. 返回:
   ...
   （MVP 跳过步骤 2 ClamAV 扫描，scan_status 直接设为 "clean"）
```
...
POST   /api/v1/knowledge-bases/{id}/documents     # R8-3
       Body: multipart
       → 202 { document_id, file_id, scan_status, index_status }
       （完整流程：scan_status="pending", index_status="pending_scan"；
         MVP 跳过 ClamAV：scan_status="clean", index_status="pending"）
...
> **MVP 简化**：ClamAV 安全扫描推迟到 post-MVP 实现。MVP 中上传即设 `scan_status=clean`、`index_status=pending`（跳过 `pending_scan` 状态），文件直接进入索引流水线。MIME 白名单与文件大小校验仍在上传时同步执行。
